...
SELECT
  ...
  END AS match_reason
FROM pets p
JOIN pet_products pr ON p.animal = pr.species
WHERE
  (pr.breeds_allowed IS NULL OR p.breed = ANY(pr.breeds_allowed))
  AND (pr.allergy_warning IS NULL OR NOT (pr.allergy_warning && p.allergies))
  AND (pr.medical_restriction IS NULL OR NOT (pr.medical_restriction && p.medical_conditions));