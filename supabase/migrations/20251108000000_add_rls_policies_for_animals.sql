...
-- Add RLS policies only if the table contains a user identifier column.
-- This avoids migration failures in schemas where the field is named differently.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    FOR UPDATE
    TO authenticated
    USING (auth.uid() = COALESCE(user_id, owner_id, created_by))
    WITH CHECK (auth.uid() = COALESCE(user_id, owner_id, created_by));
