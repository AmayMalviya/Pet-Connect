...
void main() {
  setUpAll(() async {
    ...
    // PetProfileScreen attempts to fetch breed info on startup.
  ...
  group('PetProfileScreen Simple Widget Tests', () {
    testWidgets('1. Should render the main Scaffold and AppBar with pet name', (WidgetTester tester) async {
      await tester.pumpWidget(MaterialApp(home: PetProfileScreen(pet: DummyPet())));

      expect(find.byType(Scaffold), findsOneWidget);
      expect(find.text('Buddy'), findsOneWidget); // Verifies the pet name displays in the AppBar
    });
